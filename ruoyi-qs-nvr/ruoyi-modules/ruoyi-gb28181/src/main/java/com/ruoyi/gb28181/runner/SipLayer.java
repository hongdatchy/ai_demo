...
import com.ruoyi.gb28181.config.DefaultProperties;
import com.ruoyi.gb28181.config.SipConfig;
import com.ruoyi.gb28181.config.UserSetting;
import com.ruoyi.gb28181.factory.GbStringMsgParserFactory;
...
public class SipLayer implements CommandLineRunner {
    ...
    private final List<Integer> monitorPorts = new ArrayList<>();
    ...
    public Integer getLocalPort(Integer deviceLocalPort) {
        if (monitorPorts.size() == 1) {
            return monitorPorts.get(0);
        }
        if (!ObjectUtils.isEmpty(deviceLocalPort)) {
            return deviceLocalPort;
        }
        SipProviderImpl udpSipProvider = getUdpSipProvider();
        return getUdpSipProvider().getListeningPoint().getPort();
    }
}
