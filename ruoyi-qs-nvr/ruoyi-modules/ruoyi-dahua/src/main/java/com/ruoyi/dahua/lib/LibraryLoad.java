package com.ruoyi.dahua.lib;

import cn.hutool.core.io.resource.ClassPathResource;
import com.ruoyi.dahua.common.osSelect;
...
public class LibraryLoad {
  ...
  public static String getLoadLibrary(String libraryName) {
    ...
    if (dynamicParseUtil == null) {
      try {
        ClassPathResource resource = new ClassPathResource("res\\dynamic-lib-load.xml");
        String path = resource.getPath();
        dynamicParseUtil =
            new DynamicParseUtil(
                LibraryLoad.class.getClassLoader().getResourceAsStream(path));
        if (!written) {
    ...
    if(osSelect.isWindows()){
      EXTRACT_PATH = System.getProperty("user.dir") + "\\ruoyi-modules\\ruoyi-dahua\\lib\\win\\";
    }
    if(osSelect.isLinux()){
      EXTRACT_PATH = System.getProperty("user.dir") + "/ruoyi-modules/ruoyi-dahua/lib/linux/";
    }
