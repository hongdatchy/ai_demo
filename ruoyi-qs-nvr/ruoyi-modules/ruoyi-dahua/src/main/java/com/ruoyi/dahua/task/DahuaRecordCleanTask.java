...
@Slf4j
@Component
public class DahuaRecordCleanTask {
    ...
    public void cleanOldRecordFiles() {
        log.info("开始清理大华临时录像文件...");
        try {
            String recordBasePath = filePath + "/dahua/record";
            ...
            if (recordDir.exists() && recordDir.isDirectory()) {
                ...
                if (deviceDirs != null) {
                    ...
                    log.info("大华临时录像文件清理完成，共删除 {} 个目录", deletedCount);
                }
            }
        } catch (Exception e) {
            log.error("大华临时录像文件清理异常", e);
        }
    }
